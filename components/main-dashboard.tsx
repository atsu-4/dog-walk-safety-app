...
export function MainDashboard({ appState, updateAppState, toggleWalkState, sensorData }: MainDashboardProps) {
  ...
  const getWalkStatus = (temp: number) => {
    if (temp == null || isNaN(temp)) return "safe"
  ...
  const recommendations = {
    ...
    caution: {
      ...
      en: [
        "Walk in shaded areas or on grass",
        "Keep walks to 15–20 minutes",
        "Use boots or paw balm to protect your dog's paws",
      ],
      ko: [
        "그늘이나 잔디밭에서 산책하세요",
        "산책은 15~20분 이내로 하세요",
        "강아지 발을 보호하는 신발이나 발 패드 전용 밤을 사용하세요",
      ],
    ...
    danger: {
      ...
      en: [
        ...
        "Pavement temperature is too high for walking",
      ...
      <Card>
        <CardContent className="p-4">
          ...
          <div className="space-y-2">
            {recommendations[status as keyof typeof recommendations][
              appState.language as keyof (typeof recommendations)[typeof status]
            ].map((rec, index) => (
